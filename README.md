...
- Search/replace
- Word commands: transpose, capitalize, move, delete
- Rectangle commands: copy, open, clear, paste
- Commands for whitespace: delete around points, delete empty lines
...
- Search Forward Regexp (`ctrl-alt-s`). The written text is interpreted as regular expression, and the search direction is forward in the
  editor.
...
- Clear rectangle (`ctrl-x alt-c`). Create a blank rectangle form the current selection by replacing all text within the rectangle with
  space.
...
When using sticky selection, Exchange Point and Mark (`ctrl-x ctrl-x`) switches the position of the selection start and the current caret.
The
...
Recenter (`ctrl-l`) and reposition (`alt-r`) cycles the location of the caret between three different locations: middle, top and bottom of
the
